...
samples_folder_path = sys.argv[1]


def merge_aligns(out_path,file_list,base_path):
    seq_dict = {}
    references = []
    lengths = []
...
# samples_folder_path = "/Users/ernesto/Desktop/colabo/Chantal/tRNA_is_life/data/example_folder"

folders = [x for x in os.listdir(samples_folder_path) if (os.path.isdir(os.path.join(samples_folder_path,x))
                                                          and os.path.exists(os.path.join(samples_folder_path,x,"GRCh38_p10_genomic_tRNA")))]

print(folders)

for folder in folders:
    folder_path = os.path.join(samples_folder_path,folder,"GRCh38_p10_genomic_tRNA")
    ...
    for x,y in anti_dict.items():
        ...
        merge_aligns(file_path,y,folder_path)
        print("merged " + x)
